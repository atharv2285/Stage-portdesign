...
app.get('/api/github/repos', async (req, res) => {
    try {
        ...
        const octokit = new rest_1.Octokit({ auth: userToken });
...
app.post('/api/linkedin/profile', async (req, res) => {
    try {
        ...
        const response = await fetch(`https://fresh-linkedin-profile-data.p.rapidapi.com/get-profile-data-by-url?url=${encodeURIComponent(profileUrl)}`, {
            method: 'GET',
            headers: {
